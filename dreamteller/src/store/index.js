  ...
  actions: {
    async generateStory({ commit }, storyPrompt) {
      try {
        ...
        for (let i = 0; i < story.scenes.length; i++) {
          ...
          const imagePrompt = `${storyPrompt.setting}, ${scene.text}, ${storyPrompt.style}`
          scene.imageUrl = await imageService.generateImage(imagePrompt)
        }
    ...
    async regenerateSceneImage({ commit, state }, sceneIndex) {
      try {
        ...
        const scene = state.currentStory.scenes[sceneIndex]
        const imagePrompt = `${state.currentStory.prompt.setting}, ${scene.text}, ${state.currentStory.prompt.style}`
        const newImageUrl = await imageService.generateImage(imagePrompt)
